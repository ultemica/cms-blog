...
function FeedPost({ post }: { post: AppBskyFeedDefs.FeedViewPost }) {
  ...
  return (
    <div className='bg-[#202040] dark:bg-[#202040] bg-[#f5f5fa] border-2 border-[#38c8ff] dark:border-[#38c8ff] border-[#007acc] rounded-none p-[14px] relative text-[15px] leading-[1.6] break-words font-dotgothic16 select-none text-[#e0e0e0] dark:text-[#e0e0e0] text-[#222]'>
      ...
      <div
        className='mb-2 whitespace-pre-wrap text-[#e0e0e0] dark:text-[#e0e0e0] text-[#222]'
        style={{ color: 'var(--post-text-color, #222)' }}
      >
        {/* ライトモード時はtext-[#222]、ダークモード時はtext-[#e0e0e0] */}
        <span className='dark:text-[#e0e0e0] text-[#222]'>{text}</span>
      </div>
      <div className='text-[13px] text-[#38c8ff] dark:text-[#38c8ff] text-[#007acc] text-right font-dotgothic16 font-bold tracking-[1px] mt-1'>
        {createdAt
...
export default async function Page() {
  const posts: AppBskyFeedDefs.FeedViewPost[] = await fetchBlueskyPosts()
